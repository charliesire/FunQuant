...
We can first perform quantization without considering the probabilistic distribution of the inputs.
...
res_proto_1 = proto_map_algo(gamma = gamma, outputs) 

for(i in 1:length(res_proto_1$gamma)){
  print(plot_map(res_proto_1$gamma[[i]]))
}

print(res_proto_1$probas)
```
...
res_proto_2 = proto_map_algo(gamma = gamma, outputs, density_ratio = density_ratio) 

for(i in 1:length(res_proto_2$gamma)){
  print(plot_map(res_proto_2$gamma[[i]]))
}
