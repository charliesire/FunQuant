...
#' @param bias A vector indicating the bias that came out when computing the importance sampling estimators of the membership probabilities. Each element of the vector is associated to a Voronoi cell. Default NULL.
...
#' @param cells The Voronoï cell numbers that are investigated.

#' @return The centroid and the probability mass of each probability cell
#' @import abind

compute_centroids_and_proba = function(data, cell_numbers, method_IS = "unique", sampling_cells = 1:length(data), density_ratio = rep(1, dim(data)[length(dim(data))]), bias = NULL, batch = FALSE, cells = NULL){
  ...
  if(is.null(cells)){cells = 1:length(unique(unlist(cell_numbers)))}
  ...
  for(j in cells){
    ...
    else{denominator = estim_denom_centroid(density_ratio = density_j, cell_numbers = cell_numbers_j, cell = j, bias = bias[which(cells==j)])}
    centroids = c(centroids, list(numerator/denominator))
