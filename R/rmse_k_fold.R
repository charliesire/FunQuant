...
rmse_k_fold = function(outputs, nb_folds, ncoeff_vec,npc_vec, outputs_pred = NULL, return_pred = FALSE, only_positive = FALSE, formula = ~1,design, covtype="matern5_2", wf = "d4", boundary = "periodic",J=1,
  ...
  if(is.null(bool_outputs_pred)){
    ...
    outputs_pred = list()
    for(k in 1:nb_folds){
      outputs_pred_list[[k]] = rmse_training_test(outputs_train = asub(outputs,dims = length(dim(outputs)), idx = which(folds != k)),outputs_test = asub(outputs,dims = length(dim(outputs)), idx = which(folds == k)), ncoeff_vec = ncoeff_vec,npc_vec = npc_vec, return_pred = TRUE, only_positive = only_positive,formula = formula,design_train = as.data.frame(design[folds !=k,]), design_test = as.data.frame(design[folds == k,]), covtype=covtype, wf = wf, boundary = boundary,J=J,
