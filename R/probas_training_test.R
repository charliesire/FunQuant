...
probas_training_test = function(outputs_train,outputs_test, density_ratio, prototypes, distance_func = function(A1,A2){return(sqrt(sum((A1-A2)^2)))},model_tuning = NULL, ncoeff_vec,npc_vec, return_pred = FALSE, outputs_pred = NULL, only_positive = FALSE,design_train, design_test,kernel="matern5_2", wf = "d4", boundary = "periodic",J=1,
  ...
  if(is.null(outputs_pred)){
    if(is.null(model_tuning)){model_tuning = create_models_tuning(outputs = outputs_train, ncoeff_vec = ncoeff_vec, npc = max(npc_vec), design = design_train, kernel=kernel, wf = wf, boundary = boundary,J=J,
  ...
  for(i in 1:nrow(grid_cv)){
    ...
    probas_pred_cv = get_probas(density_ratio = density_ratio, data = outputs_pred_i, prototypes = prototypes, distance_func = distance_func, cells = 1:length(prototypes), bias = bias)
