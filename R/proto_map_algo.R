...
proto_map_algo = function(gamma, outputs, method_IS = "unique", density_ratio = rep(1, dim(outputs)[length(dim(outputs))]), budget = 10^3, threshold = 0, distance_func, print_progress = FALSE, trace = FALSE, bias = rep(0,length(gamma))){
  ...
  for (i in 1:budget){
    ...
    if(method_IS == "unique"){cell_numbers = get_cell_numbers(outputs,gamma,distance_func = distance_func)} #Associate each map to its Voronoi cell
    else if(method_IS == "percell"){cell_numbers = lapply(1:length(outputs), function(h){get_cell_numbers(outputs[[h]],gamma,distance_func = distance_func)})}
