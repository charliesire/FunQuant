...
proto_map_algo = function(gamma = NULL, nb_cells = NULL, outputs, multistart = 1, method_IS = "unique", density_ratio = rep(1, dim(outputs)[length(dim(outputs))]), budget = 10^3, threshold = 0, distance_func = function(A1,A2){return(sqrt(sum((A1-A2)^2)))},print_progress = FALSE, trace = FALSE, all_starts = FALSE,bias = NULL, index_sampling_error = NULL, seed = NULL){
  ...
  for(it in 1:multistart){
    ...
    for (i in 1:budget){
      ...
      if(method_IS == "unique"){
        cell_numbers = get_cell_numbers(outputs,gamma_it,distance_func = distance_func)
        if(length(table(cell_numbers))<length(gamma_it)){stop("One of the initial Voronoi cells is empty")}
      }
    ...
    gamma_it = sort_gamma(gamma_it)
    error = quanti_error(outputs = outputs, gamma = gamma_it, density_ratio = density_ratio, distance_func = distance_func)
