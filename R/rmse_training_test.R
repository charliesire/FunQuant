#' @title Computation of the Root Mean Square Error at each pixel of the outputs on a validation dataset, for different values of ncoeff and npc
#'
...
rmse_training_test = function(outputs_train,outputs_test, model_tuning = NULL, ncoeff_vec,npc_vec, return_pred = FALSE,outputs_pred = NULL, only_positive = FALSE,design_train, design_test,
  ...
  if(is.null(outputs_pred)){
    if(is.null(model_tuning)){model_tuning = create_models_tuning(outputs = outputs_train, ncoeff_vec = ncoeff_vec, npc = max(npc_vec), design = design_train, kernel=kernel, wf = wf, boundary = "periodic",J=J,
